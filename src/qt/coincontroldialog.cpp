...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            ...
            // usually, the result will be an overestimate within a couple of dragons so that the confirmation dialog ends up displaying a slightly smaller fee.
    ...
    int nDisplayUnit = BitcoinUnits::LYC;
    ...
    // how many dragons the estimated fee can vary per byte we guess wrong
    double dFeeVary = (nBytes != 0) ? (double)nPayFee / nBytes : 0;

    QString toolTip4 = tr("Can vary +/- %1 dragon(s) per input.").arg(dFeeVary);
