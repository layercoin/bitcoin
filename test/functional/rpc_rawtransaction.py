...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        blockcount = 1
        while blockcount > 0:
            block1 = self.nodes[2].generate(blockcount)
            blockcount -= len(block1)
        blockcount = 1
        while blockcount > 0:
            block2 = self.nodes[2].generate(blockcount)
            blockcount -= len(block2)
        self.sync_all()
        # We should be able to get the raw transaction by providing the correct block
        gottx = self.nodes[0].getrawtransaction(tx, True, block1[0])
        ...
        assert_raises_rpc_error(-5, "No such transaction found", self.nodes[0].getrawtransaction, tx, True, block2[0])
        ...
        self.nodes[0].invalidateblock(block1[0])
        gottx = self.nodes[0].getrawtransaction(txid=tx, verbose=True, blockhash=block1[0])
        assert_equal(gottx['in_active_chain'], False)
        self.nodes[0].reconsiderblock(block1[0])
        assert_equal(self.nodes[0].getbestblockhash(), block2[0])
