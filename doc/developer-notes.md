...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_layercoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_layercoin --log_level=test_suite
$ valgrind -v --leak-check=full src/layercoind -printtoconsole
```
...
To profile a running layercoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep layercoind` -- sleep 60
```
...
- ThreadSocketHandler : Sends/Receives data from peers on port 9218.
...
- ThreadRPCServer : Remote procedure call handler, listens on port 9217 for connections and services them.
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `layercoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `layercoin-cli` and the GUI debug console use this table to determine how to
